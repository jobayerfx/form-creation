    ...
    // $(function() {
        ...
        $('#create-form').validate({
            'submitHandler': function (form) {
                saveForm(form, function ($form) {
                    ...
                    $form.find('#form-preview').removeClass('d-none').attr('href', '{{ route('forms.preview', $form->code) }}');
        ...
        function completeTemplateInterfaceFeatures($questions) {
            ...
            $bootstrap_select.change(function () {
                var $select = $(this);
                var labels = $select.closest('.card-body').find('.select-labels');
            ...
            add_option_buttons.each(function (index) {
                ...
                $add_option_button.on('click', function (event) {
                    ...
                    var options_wrapper = $(event.target).closest('.card-body').find('div.options-wrapper');
                    var option_count = options_wrapper.children(':not(.d-none)').length + 1;

                    if (option_count < max_option_count) {
                        // Create a new wrapper
                        var new_option = options_wrapper.find('.d-none').clone().removeClass('d-none');
